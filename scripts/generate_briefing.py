...
调用 Gemini API + Google Search 生成简报，
...
请按步骤生成今日市场简报：
1. 使用搜索工具搜索今日焦点个股新闻（从 top_movers 挑选3-5只最有新闻价值的）
2. 使用搜索工具搜索今日重大财经新闻（6-10条）
3. 使用搜索工具搜索华尔街机构近期观点（5条）
4. 使用搜索工具搜索明日重要事件
5. 生成完整的 Markdown 简报
...
"""
    ...
    print("  Calling Gemini API (gemini-2.0-flash) with Google Search grounding...", flush=True)
    ...
    if not briefing:
        raise ValueError("Gemini returned empty briefing")
